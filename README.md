...
**当前版本：v1.7.14** · [下载最新 fpk](https://github.com/han951meng/nasdash/releases/latest)
...
- 传感器芯片：Nuvoton NCT6797D（nct6775 驱动）—— 本机**唯一真实风扇控制器**，暴露 5 个 PWM 通道（pwm1–pwm5）；本机并无 it87 芯片，「it8772 风扇」字样来自旧 FanControlServer 配置残留
- 风扇接线（已逐一拉满转速肉眼核对）：
  - **CPU_FAN1** → CPU 散热器风扇（有测速）
  - **SYS_FAN1** → 主板/芯片组散热风扇（有测速）
  - **SYS_FAN2** → 阵列卡风扇（**该扇缺测速线，面板显示「无转速信号」，但能正常调速**）
  - **SYS_FAN3** → 硬盘散热风扇（一分二转接两把 PWM 风扇，有测速）
  - **PUMP_FAN1** → 空口（风冷机未接水冷泵，无风扇，面板可一键隐藏）

### 常见「看起来像 bug」的正常现象
1. **风扇显示「无转速信号」**：该通道接的风扇没接测速线（3 针/缺 tach），或是空接口（如 PUMP_FAN1）。面板仍能调速，只是读不到 RPM 数字，可点「隐藏」收起。
2. **hwmon 编号跨重启变化**：内核给风扇控制器分配的 `hwmonN` 路径（如 nct6797 在 hwmon3↔hwmon4 间漂移）不固定。面板已自动校正——调速请求按通道序号(idx)命中真实通道，不受 hwmon 编号变化影响；自定义名称/隐藏也按 idx 兜底，重启后不乱。
3. **名字显示成「it8772 风扇 N」**：读取到机器上旧 FanControlServer 配置残留标签。可在风扇卡片「自定义名称」改成你喜欢的（如 CPU 风扇），按安装实例持久化，硬件无关不写死机型。
4. **手动调速后停在目标值、不再回弹**：设计行为——拖动滑块即切换为手动控制并平滑过渡到目标占空比，转速数字继续爬升；点「恢复自动」才交还主板/内核自动控温（nasdash 自带保守曲线钳制 30~70%）。
5. **滑块最低 10%**：手动调速下限为 10%（该硬件 10% 仍可运转），非 0%。
...
### v1.7.14
- 修复部分风扇调速失效(CPU_FAN1/SYS_FAN1等)：GUI滑块hwmon与后端控制路径因FanControlServer残留旧hwmon路径+内核hwmon重排而不一致，调速请求命中不到真实通道(停在自动曲线35%)；get_system改以实时枚举为权威hwmon，api_fan_set按idx校正hwmon，双保险；README新增实测硬件配置避坑说明
