...
**当前版本：v2.0.5** · [下载最新 fpk](https://github.com/han951meng/nasdash/releases/latest) · [📖 操作手册](docs/使用手册.md) · [💬 飞牛社区讨论帖](https://club.fnnas.com/forum.php?mod=viewthread&tid=67060)
...
### 环境要求
- 飞牛OS（fnOS），应用固定监听端口 **9800**（仅本机内部监听，对外经飞牛统一网关访问）
...
左侧边栏共九个功能模块（硬件配置检测 / 阵列卡 / 硬盘 SMART / 系统资源 / 历史趋势 / 风扇控制 / 存储卷 / Docker / 控制与自动化），外加「操作手册」和「关于 nasdash」两个常驻入口，覆盖 NAS 硬件运维核心场景：

### 🏠 硬件配置检测（首页仪表盘）
单页总览，进入即看全貌：
- 顶部状态栏：实时概览（阵列卡 / 硬盘 / Docker / 系统负载）
- 系统配置：处理器（含型号、核心/线程）、显卡（核显 / NVIDIA / AMD 独显自动识别）、内存（品牌取自 SPD 直读，区分模组厂与颗粒厂）、主板型号（DMI 直读品牌 / 型号 / 版本 / BIOS / 芯片组）、系统信息（含运行时间）、网络（网卡 IP / 速率 / 状态）
...
- **硬盘自检（A/B 双档）**：可对盘体检或排查坏道——A 档 SMART 长自检（只读、不伤数据，可中止、实时看进度与 PASS/FAIL 结论）；B 档坏块慢扫（写满清空数据，仅放行独立盘，在用盘按钮自动置灰、硬点被拦，避免误清）
...
### 🔔 控制与自动化
- **告警规则**：CPU / 主板·芯片组 / 硬盘温度阈值 + 内存占用上限 + 硬盘 SMART 健康（pre-fail / 异常）告警；超阈值自动触发
...
- 服务端口 9800（在 manifest 的 service_port 固定声明），修改需同步更新 manifest 的 service_port
...
| 改动点 | 说明 / 效果 |
|---|---|
| CPU 使用率修准 | 修复偶发显示 0% 与瞬时 100%/50% 尖刺，改为后台固定 1 秒窗口采样，数值稳定、对齐飞牛原生显示 |
| 系统负载显示 | 格式化为两位小数，不再出现超长小数（如 0.57763671875） |
| 阵列卡温度 | ROC 温度并入「温度监控」页与顶部统计，温度墙一眼可见 |
| 网络卡片刷新 | 抽独立接口 + 前端 5 秒实时刷新；顶部「网络」格子同步实时刷新（此前只有整页刷新才变） |
| 温度监控页 | 精简：移除与温度墙重复的左侧传感器列表，只保留温度墙网格 |
| 系统资源页 | 参考 Windows 任务管理器风格重构 CPU/内存/网络卡片（大百分比 + 进度条） |
| 网络速率统计 | 修复不刷新、且把虚拟网卡/网桥重复合计的问题，现每 2 秒实时刷新并过滤 ovs 虚拟口 |
| 网络卡片精简 | 按反馈去掉每个网口下方的实时速率小字（保留「更新于」时间戳） |

### v2.0.3
- 修复应用中心打开 404：彻底改走 cgi 反代模式（`/cgi/ThirdParty/.../index.cgi/` + 内部转发到 127.0.0.1:9800），对齐 fnpackup 等稳定第三方应用，不再依赖会被飞牛周期性清空的统一网关入口
- 删除网关自愈看门狗（cgi 模式不需要，cmd/main 从 173 行精简到 97 行）
- 一键健康报告「复制诊断信息」改为「查看运行日志」弹窗：错误/警告置顶着色，看完再决定是否复制；顶部重复的「导出健康报告」按钮已合并

### v2.0.2
- 温度监控独立页去重优化（移除与系统资源页重复的 CPU/内存指标，新增最高/平均温度、测点计数与状态徽章）；三档明暗主题跟随飞牛；SVG 线框图标对齐飞牛原生风格；真实 CPU 使用率采集；磁盘实时监控新增每块盘占用率(busy%)与待机状态显示
...
**界面大版本，主打换新装 + 向原生看齐 + 加功能。**

**一、界面换新装（2.0 视觉）**
- 11 个板块图标全部重画：蓝紫渐变圆角方块、真透明背景，亮色/暗色模式都能自然融入。
- 应用中心图标换成「关于 nasdash」图。
- 整体配色往飞牛原生界面靠：卡片、侧栏、选中态用飞牛同款蓝，看着更统一干净。

**二、严格按飞牛规范开发，向原生应用对齐**
- 2.0 起，nasdash 严格遵循飞牛应用开发规范：UI 组件、交互逻辑、配色体系全面向飞牛原生应用看齐，用起来和飞牛自家应用一个调性。

**三、新加入的功能**
- **硬盘 SMART 自检（A 档）**：在「硬盘 SMART」板块可以直接发起 SMART 长自检、随时中止、实时看进度和结果，不用再跑去敲命令。
- **坏块慢扫（B 档）**：可对独立盘（未挂载、不在阵列/LVM）执行坏块扫描，彻底排查盘体坏道；在用盘按钮自动置灰，硬点也会被拦下，避免误清数据。
- **风扇控制接管开关**：新增一键接管 / 交还风扇调速的总开关。接管后由 nasdash 按温度自动调，交还后交给飞牛原服务；界面做了防卡死保护。

---

### 📦 历史版本归档

v1.9.x 及更早版本已归档至 [`releases/`](releases/) 文件夹，并保留在 GitHub Releases 中。如需旧版 fpk 或对应操作手册，请前往查看。
