...
**当前版本：v1.9.9** · [下载最新 fpk](https://github.com/han951meng/nasdash/releases/latest) · [📖 操作手册](docs/使用手册.md)
...
### v1.9.9
- 修复温度墙高温显示：恢复显示 100°C 以上温度探头（v1.9.5 起被过滤隐藏的回归）


### v1.9.8
- 硬盘休眠更友好：降低 SMART 采集频率，休眠中的硬盘不再被频繁唤醒，延长寿命、降低功耗
- 页面刷新不再闪烁跳动：数据更新只刷变化部分，滚动位置与展开状态都保留
- 打开秒出、不再卡「加载中」：刷新先用上次数据立即显示，最新数据后台静默刷新后无缝替换


### v1.9.7
- 修复网卡重复显示与主板温度读错，风扇调速方式改为只读显示，并补全网卡信息(照飞牛样式)；另优化打开速度


### v1.9.6
- 修复飞牛网卡显示无IP、实时流量刷新慢，以及风扇单标注被清空问题
