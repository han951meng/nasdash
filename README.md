...
**当前版本：v1.9.5** · [下载最新 fpk](https://github.com/han951meng/nasdash/releases/latest) · [📖 操作手册](docs/使用手册.md)
...
### v1.9.5
- 风扇接口 PWM/DC 调速方式识别与切换：面板显示每个风扇口是 4 针 PWM 还是 3 针 DC 调速，支持直接切换（能否切换取决于主板硬件，切不动会明确提示原因）
- 风扇「无反馈线」标注：2/3 针无测速线的风扇不再误显 0 转，改为明确标注「无反馈线（正常）」；无反馈线且处于 PWM 模式时提示百分比可能不起作用
- 显卡监控大升级：温度墙中文名、显卡卡片结构化+收纳（更多参数）、AMD 独显显存位宽识别、实时功耗与驱动版本、显卡名精简


### v1.9.4
- 硬件配置检测修复（芯片组识别未知/显卡智能区分核显独显）


### v1.9.3
- 风扇温控修复（关闭温度源生效/隐藏按钮挪位/0%仍转折叠提示）及目标标签空白、手册即时显示优化


### v1.9.1
- 修复 M.2 固态盘误判使硬盘休眠后风扇仍不停止；导航栏改为常驻可独立滚动，滚动条闲置时自动隐藏


### v1.9.0
- 修复风扇控制社区反馈：设温度联动控速后手动调速失效（手动优先于温控，点「恢复自动」交还温控）
- 修复反向互锁：手动调速后改设温度联动控速无法应用（存规则时自动清除残留手动档，温度联动立即接管）
- 状态标签正确显示温控来源（自动·硬盘温控 / 自动·CPU/主板温控）
- 同名风扇自动合并为一张卡并同步改名/隐藏/调速（解决「一个风扇显示成两张卡」）
- 新增「设备诊断包」：一键复制风扇通道原生态文本，贴论坛即可定位
