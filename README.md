...
**当前版本：v1.9.0** · [下载最新 fpk](https://github.com/han951meng/nasdash/releases/latest) · [📖 操作手册](docs/使用手册.md)
...
### v1.8.10
- 修复顶部「散热风扇」误报异常（空置口/无转速风扇不再误判）


### v1.8.9
- 修复风扇温控规则在系统重启后失效（表现为部分风扇停不下来/只控一组）：改为按风扇物理位置绑定规则，不再依赖会变的接口编号


### v1.8.8
- 修复风扇两组独立温控同时生效、硬盘休眠/空闲停转、同名风扇串号、低温真正停转至0% 等社区反馈问题


### v1.8.7
- 风扇控制新增「逐风扇温度联动控速」：每台风扇可单独选温度源（硬盘/CPU/主板）并独立设曲线，支持同温源不同曲线（如一台 30–45°C、另一台 45–60°C）
- 新增「低于开转温度即停转（强制 0%）」选项：实现「高温才启暴力扇、平时彻底不转」
- 风扇控制三处修复：手动调速可设到 0%（真正停转）；重名风扇卡片不再串号（改名/隐藏单台互不影响）；硬盘温控新增「连续空闲 N 分钟即停转风扇」选项（与休眠二选一满足即停，解决设了休眠风扇却一直转）


### v1.8.6
- 修复硬盘温控：SAS/固态盘正确识别为常驻，休眠停转不再失效；温控配置读取修复（重启不再丢）；操作手册应用内就地显示
