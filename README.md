...
- 系统配置：处理器（含型号、核心/线程）、显卡（核显 / NVIDIA / AMD 独显自动识别）、内存（品牌取自 SPD 直读，区分模组厂与颗粒厂）、主板型号（DMI 直读品牌 / 型号 / 版本 / BIOS / 芯片组）、系统信息（含运行时间）、网络（网口 Tab：每口显示 IP / IPv4/IPv6 / MAC / 协商速率 / 双工 / MTU / 厂商 / 驱动 / PCI 位置 / UP-DOWN 状态，实时收发速率 5 秒刷新）
