...
- 阵列卡 & 磁盘：卡型号与物理盘、SMART 健康速览（注：v2.0.3 后阵列卡信息已合并进首页仪表盘，不再独立成页）

### 🃏 阵列卡（自动检测卡类型，v2.0.3 后合并进首页）
...
### 🌡️ 温度监控
- 温度墙网格：按 核心/NVMe/SSD/机械 分组展示，统一高度对齐
- 实时刷新：每 5 秒就地更新温度数字（此前 30 秒整页刷新才变）
- 中文传感器代号：CPUTIN→CPU 温度等，悬停看原名
- 温度阈值设定：可对关键测点设上限，超阈值标红提示
