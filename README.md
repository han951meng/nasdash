...
左侧边栏共十个功能模块（硬件配置检测 / 系统资源 / 温度监控 / 历史趋势 / 硬盘 SMART / 存储卷 / 风扇控制 / Docker / 端口占用 / 控制与自动化），外加「操作手册」和「关于 nasdash」两个常驻入口，覆盖 NAS 硬件运维核心场景：
...
- **MegaRAID 阵列卡**（如 LSI 9271-8i，IR/RAID 模式）：走 `storcli /c0 show` 显示完整信息——型号 / 序列号 / SAS 地址 / 固件版本 / BIOS 版本 / 驱动版本 / PCI 地址、CacheVault 缓存电池状态、**ROC 芯片温度**、JBOD 物理盘列表（槽位 / 品牌 / 型号(+特性徽章) / 接口 / 容量 / 状态 / 转速）。双磁臂（双执行器）盘会自动合并成一块，显示整盘标称容量并标注每执行器容量
...
### 🔌 端口占用
- 列出 NAS 上**所有正在监听的 TCP / UDP 端口**：协议 / 本地地址:端口 / 状态 / 占用进程 / PID / 来源
- **自动识别占用者是谁**：飞牛应用中心安装的应用（显示应用真名与应用自带图标，如「飞邮」「MiProxy」「WorkBuddy 积分助手」）、Docker 容器（商店安装的容器同样显示真名）、飞牛系统服务（配中文说明，如「飞牛文件共享 (SMB)」「飞牛 Web 管理后台」）；由内核线程持有的端口（如 NFS 2049）标注「内核」
- **可达范围三级徽章**：对外（局域网可访问）/ 容器网（仅宿主机与容器内部可访问）/ 仅本机（只有 NAS 自己能连）
- **搜索 + 来源筛选**：可按端口 / 进程 / 应用名 / 来源说明搜索（支持中文应用名），或只看「应用 / 容器 / 系统」某一类
- **一键释放**：释放端口会向占用进程发送终止信号（SIGTERM），前端二次确认 + 后端拒绝终止系统关键进程（nasdash 自身 / systemd / dockerd / sshd / nginx 等）；同一端口有多个进程时逐行列出，不会被漏掉
...
- 【论坛反馈】补全「阵列卡背后的物理盘在『物理硬盘信息』页看不到」的修复：v2.3.0 后端已把组 RAID 后系统无 /dev 节点的物理盘加进列表，但前端用空盘符当唯一身份证，导致多块盘只渲染成一张（视觉上=消失）。现给每块盘分配阵列卡唯一身份证并按槽位命名，物理盘全部可见、点开可看通道与健康；无 /dev 节点故不支持自检（已禁用按钮并说明）。检测页盘列表同步补全盘名。

### v2.3.0
- 新增端口占用页（列出全部监听端口，自动识别是哪个应用 / 容器 / 系统服务在占用，可一键释放）、网卡唤醒开关、硬盘健康分级与缺陷增长趋势、硬盘自定义命名、存储卷映射树、温度预警红线、内存详细信息弹窗（点内存卡查看颗粒厂商/生产日期/序列号/Rank/位宽等），健康报告整份重做并新增脱敏导出；新增网卡未跑满提示（支持 2.5G 但当前协商速率更低时给出瓶颈提示）；修复双磁臂硬盘被当成两块盘、健康报告把软阵列当阵列卡、温度表混入 0℃ 假读数、CPU 使用率恒空等问题；【论坛反馈】修复 LSI MegaRAID 阵列卡（如 MR9362-8i）的多项问题：硬盘型号被错识别成 U/D 怪字符、阵列卡背后的物理盘不显示、硬盘温度读不到，以及风扇温度源缺少「阵列卡」导致温度监控页/风扇控制页看不到阵列卡温度——现已修好型号解析、补回物理盘并加入阵列卡温度源。


### v2.2.0
nasdash 2.2.0 是界面底层整体换新版：全部 11 个页面换用新界面引擎（界面样式与旧版保持一致，页签切换零刷新、打开更快）；同时新增多项功能，并修复多个旧版长期存在的问题。
