...
- 修复接入网关后应用打不开（404 / Not Found）：新增网关入口自愈与常驻守护，安装、重装或系统同步后自动补正网关路由并重载，应用中心点击稳定打开；storcli 改用 usr-local-linker 自动软链，安装更稳。
- 网关用户身份鉴权加固：消费网关转发的用户 Header（X-Trim-Userid / Isadmin / Username），风扇调速与所有配置写入接口强制要求管理员身份，不信任客户端传入的用户 ID，符合飞牛统一网关安全规范。
